...
VIDEO_EXT = status_mod.VIDEO_EXT
...
def cmd_ui(args) -> int:
    from . import webui  # local import: keep cold start of other commands unchanged
    return webui.serve(port=args.port, open_browser=not args.no_open,
                       as_json=args.json)
...
def build_parser() -> argparse.ArgumentParser:
    ...
    sp = add("ui", "read-only web UI: selects gallery, cuts, inputs "
                   "(localhost, blocks until Ctrl-C)", cmd_ui)
    sp.add_argument("--port", type=int, default=8765,
                    help="port on 127.0.0.1 (default: 8765; 0 = auto)")
